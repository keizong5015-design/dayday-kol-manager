...
const STATUSES = ['全部', '洽談中', '已確認', '合作完成', '暫緩']
const SORT_OPTIONS = [
...
export default function Gallery({ influencers, loading, onGoAdmin }) {
  ...
  const clearAll = () => { setSearch(''); setSelPlatforms([]); setSelStyles([]); setStatus('全部') }
        ...
        <aside style={{ width: '220px', flexShrink: 0 }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)', position: 'sticky', top: '86px', maxHeight: 'calc(100vh - 110px)', overflowY: 'auto' }}>
            <div style={{ marginBottom: '20px' }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder='🔍 搜尋名稱...' style={inputStyle} />
            </div>
            <Section title='平台'>
            ...
            {hasFilter && <button onClick={clearAll} style={{ width: '100%', marginTop: '8px', padding: '8px', borderRadius: '8px', border: '1.5px solid #F0E8E6', background: 'transparent', fontSize: '12px', color: '#9CA3AF', cursor: 'pointer' }}>清除所有篩選</button>}
          </div>
        </aside>
      )}
      <main style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <button onClick={() => setSidebarOpen(v => !v)} style={{ background: 'white', border: '1.5px solid #F0E8E6', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', color: '#6B7280', cursor: 'pointer' }}>{sidebarOpen ? '◀ 收起篩選' : '▶ 展開篩選'}</button>
            <span style={{ fontSize: '13px', color: '#9CA3AF' }}>顯示 <strong style={{ color: '#E8523A' }}>{result.length}</strong> / {influencers.length} 位</span>
          </div>
        ...
        {loading ? (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '80px', color: '#9CA3AF' }}>
            <div><div style={{ fontSize: '32px', textAlign: 'center', marginBottom: '12px' }}>⏳</div><div>載入中...</div></div>
          </div>
        ) : result.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px', color: '#9CA3AF' }}>
            {influencers.length === 0 ? (
              <>
                ...
                {onGoAdmin && (
                  <button onClick={onGoAdmin} style={{ padding: '10px 24px', borderRadius: '10px', background: '#E8523A', color: 'white', border: 'none', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>＋ 新增第一位 KOL</button>
                )}
              </>
            ) : (
              <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: '18px' }}>
            {result.map(inf => <KolCard key={inf.id} influencer={inf} />)}
          </div>
        )}
      </main>
    </div>
